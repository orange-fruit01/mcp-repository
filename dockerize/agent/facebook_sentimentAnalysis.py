...
def fetch_comments_from_db(state: SentimentState) -> dict:
    try:
        ...
        df = pd.read_sql_query(query, engine, params={"platform": "facebook"})
        return {
            "comments_df": df,
            "status": "comments_fetched",
            "message": f"Fetched {len(df)} comments from DB"
        }
    except Exception as e:
        return {
            "comments_df": pd.DataFrame(),
...
def analyze_and_insert_sentiment(state: SentimentState) -> dict:
    try:
        ...
        for post_id, group in grouped:
            post_url = f"https://www.facebook.com/me/posts/{post_id}"
            platform = "facebook"
            user_id = 1
...
#     print("Facebook Sentiment Analysis Agent (local mode)")
#     try:
#         result = graph.invoke({})
#         print("\n" + "="*50)
